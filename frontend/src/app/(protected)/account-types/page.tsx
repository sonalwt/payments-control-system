...
import type { AccountType, Paginated } from '@/types/domain';
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-2">
        ...
        <Input id="name" placeholder="Current Account" {...register('name')} />
...
export default function AccountTypesPage(): React.ReactElement {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [KEY, params],
    queryFn: () => api.get<Paginated<AccountType>>(`/account-types?${params}`),
  });

  const createMut = useMutation({
    mutationFn: (i: FormData) => api.post<AccountType>('/account-types', i),
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, i }: { id: string; i: FormData }) => api.put<AccountType>(`/account-types/${id}`, i),
    onSuccess: () => { void qc.invalidateQueries({ queryKey: [KEY] }); setEditing(null); notify.success('Updated'); },
    onError: (e: Error) => notify.error('Failed', e),
  });
  const deleteMut = useMutation({
    mutationFn: (id: string) => api.del<void>(`/account-types/${id}`),
    ...
    <div>
      ...
      <Card>
        ...
        <Table>
          ...
          <TableBody>
            ...
            ) : data && data.data.length > 0 ? data.data.map((at) => (
              <TableRow key={at.id}>
                ...
                <TableCell className="text-right">
                  <Button size="icon" variant="ghost" onClick={() => setEditing(at)}><Pencil className="h-4 w-4" /></Button>
                  <Button size="icon" variant="ghost" onClick={() => setDeleting(at)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                </TableCell>
              </TableRow>
            )) : (
              <TableRow><TableCell colSpan={3} className="py-12 text-center text-muted-foreground">No account types yet.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent>
          ...
          {editing && <AccountTypeForm defaultValues={editing} submitting={updateMut.isPending} onSubmit={(d) => updateMut.mutate({ id: editing.id, i: d })} />}
        </DialogContent>
      </Dialog>
      <ConfirmDelete
        open={!!deleting}
        ...
        description="This will soft-delete the account type."
