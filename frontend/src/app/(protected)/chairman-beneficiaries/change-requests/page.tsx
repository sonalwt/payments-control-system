...
import type {
  ChairmanBeneficiaryChangeRequest,
  ChairmanCrStatus,
  ChairmanCrType,
  Paginated,
} from '@/types/domain';
...
const KEY = 'chairman-beneficiary-change-requests';

const STATUS_LABEL: Record<ChairmanCrStatus, string> = {
...
const STATUS_STYLE: Record<ChairmanCrStatus, string> = {
...
const TYPE_LABEL: Record<ChairmanCrType, string> = {
  ADD: 'ADD',
  MODIFY: 'MODIFY',
  DEACTIVATE: 'DEACTIVATE',
};

const TYPE_STYLE: Record<ChairmanCrType, string> = {
...
function StatusBadge({ status }: { status: ChairmanCrStatus }): React.ReactElement {
  return (
    <span
...
function TypeBadge({ type }: { type: ChairmanCrType }): React.ReactElement {
  return (
    <span
...
function ProposedDataView({ data }: { data: Record<string, unknown> }): React.ReactElement {
...
interface DetailDialogProps {
  request: ChairmanBeneficiaryChangeRequest;
  open: boolean;
  onClose: () => void;
...
function DetailDialog({ request, open, onClose, onActionSuccess }: DetailDialogProps): React.ReactElement {
  ...
  const verifyMutation = useMutation({
    mutationFn: () =>
      api.post(`/chairman-beneficiaries/change-requests/${request.id}/verify`, {
  ...
  const approveMutation = useMutation({
    mutationFn: () =>
      api.post(`/chairman-beneficiaries/change-requests/${request.id}/approve`, {
  ...
  const rejectMutation = useMutation({
    mutationFn: () =>
      api.post(`/chairman-beneficiaries/change-requests/${request.id}/reject`, {
  ...
  const missingDocs = needsCallback
    ? REQUIRED_DOC_CODES.filter((code) => !presentDocCodes.has(code))
    : [];

  const accountLabel =
    request.chairmanBeneficiary
      ? `${request.chairmanBeneficiary.accountHolderName} — ${request.chairmanBeneficiary.accountNumber}`
      : request.changeType === 'ADD'
        ? 'New Chairman Account'
        : '—';
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            <span>Chairman Beneficiary Change Request</span>
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-3 rounded-md border border-red-300 bg-red-50 p-3 text-red-800">
              ...
              <div className="space-y-1">
                ...
                {request.anomalyNotes
                  ...
                  : <p className="text-xs">One or more anomaly signals were triggered.</p>}
              </div>
            </div>
          )}

          {/* §9 / §6.5 — Sanctioned-country warning */}
            ...
            <div className="flex gap-3 rounded-md border border-orange-400 bg-orange-50 p-3 text-orange-900">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs">
                  ...
                  The final approver must provide a written acknowledgement before approval.
                </p>
          ...
          {/* Verification info */}
          {(['VERIFIED', 'APPROVED', 'REJECTED', 'CANCELLED'] as ChairmanCrStatus[]).includes(request.status) &&
            request.verifiedBy && (
              <section className="space-y-3">
                <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Verification
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                  <Field label="Verified By" value={request.verifier?.fullName ?? request.verifiedBy} />
                  <Field
                    label="Verified At"
                    value={request.verifiedAt ? new Date(request.verifiedAt).toLocaleString() : undefined}
                  />
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                {request.status === 'APPROVED' && (
                  <>
                    ...
                    <Field
                      label="Approved At"
                      value={request.approvedAt ? new Date(request.approvedAt).toLocaleString() : undefined}
                    />
                  </>
                )}
                {request.status === 'REJECTED' && (
                  <>
                    ...
                    <Field
                      label="Rejected At"
                      value={request.rejectedAt ? new Date(request.rejectedAt).toLocaleString() : undefined}
                    />
          ...
          {request.status === 'PENDING_VERIFICATION' && (
            <section className="space-y-3 rounded-md border p-4">
              ...
              <p className="text-xs text-muted-foreground">
                You cannot verify a request you created (maker-checker separation).
              </p>
                ...
                <div className="rounded-md border bg-muted/30 p-3 space-y-1.5">
                  ...
                  {missingDocs.length > 0 && (
                    <p className="text-xs text-red-600 mt-1">
                      All required documents must be present before verification.
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    ...
                    placeholder="Notes from account verification…"
                    rows={3}
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  {request.sanctionWarning && (
                    <div className="space-y-1.5">
                      ...
                      <Textarea
                        ...
                        placeholder="Written acknowledgement that this beneficiary is in a sanctioned country…"
                        rows={3}
                        className={!sanctionAck.trim() ? 'border-orange-400 focus-visible:ring-orange-400' : ''}
                      />
                      {!sanctionAck.trim() && (
                        <p className="text-xs text-orange-700">
                          Required before approval can be registered.
                        </p>
                      )}
                    </div>
                  )}
...
export default function ChairmanBeneficiaryChangeRequestsPage(): React.ReactElement {
  const [page, setPage] = useState(1);
  const [selected, setSelected] = useState<ChairmanBeneficiaryChangeRequest | null>(null);
  const qc = useQueryClient();

  const { data, isLoading } = useQuery({
    queryKey: [KEY, page],
    queryFn: () =>
      api.get<Paginated<ChairmanBeneficiaryChangeRequest>>(
        `/chairman-beneficiaries/change-requests?page=${page}&limit=20`,
      ),
  });
  ...
  function getAccountLabel(req: ChairmanBeneficiaryChangeRequest): string {
    if (req.chairmanBeneficiary) {
      return `${req.chairmanBeneficiary.accountHolderName} — ${req.chairmanBeneficiary.accountNumber}`;
    }
    if (req.changeType === 'ADD') return 'New Chairman Account';
    return '—';
  }
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-2">
        <Link href="/chairman-beneficiaries">
      ...
      <PageHeader
        title="Chairman Beneficiary Change Requests"
        description="Verify and approve changes to chairman-designated beneficiary accounts."
      />
            ...
            {isLoading ? (
              ...
              data.data.map((req) => (
                <TableRow key={req.id}>
                  ...
                  <TableCell className="text-sm">{getAccountLabel(req)}</TableCell>
                  ...
                  <TableCell className="text-sm">
                    {req.verifier?.fullName ?? (req.verifiedBy ?? '—')}
                  </TableCell>
                  <TableCell>
                    <StatusBadge status={req.status} />
                  </TableCell>
                  <TableCell className="text-center">
                    {(req.anomalyFlag || req.sanctionWarning) && (
                      <span title={req.anomalyFlag ? 'Anomaly detected' : 'Sanctioned country'}>
                        <AlertTriangle className={`inline-block h-4 w-4 ${req.anomalyFlag ? 'text-red-500' : 'text-orange-500'}`} />
                      </span>
                    )}
                  </TableCell>
