...
import type { Paginated, PaymentCategory } from '@/types/domain';
...
function PaymentCategoryForm({
  defaultValues, onSubmit, submitting,
}: {
  defaultValues?: Partial<PaymentCategory>;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-2">
        ...
        <Input id="name" placeholder="Trade Payments" {...register('name')} />
...
export default function PaymentCategoriesPage(): React.ReactElement {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [KEY, params],
    queryFn: () => api.get<Paginated<PaymentCategory>>(`/payment-categories?${params}`),
  });

  const createMut = useMutation({
    mutationFn: (i: FormData) => api.post<PaymentCategory>('/payment-categories', i),
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, i }: { id: string; i: FormData }) => api.put<PaymentCategory>(`/payment-categories/${id}`, i),
    onSuccess: () => { void qc.invalidateQueries({ queryKey: [KEY] }); setEditing(null); notify.success('Updated'); },
    onError: (e: Error) => notify.error('Failed', e),
  });
  const deleteMut = useMutation({
    mutationFn: (id: string) => api.del<void>(`/payment-categories/${id}`),
  ...
  return (
    <div>
      ...
      <Card>
        ...
        <Table>
          ...
          <TableBody>
            ...
            ) : data && data.data.length > 0 ? data.data.map((pc) => (
              <TableRow key={pc.id}>
                ...
                <TableCell className="text-right">
                  <Button size="icon" variant="ghost" onClick={() => setEditing(pc)}><Pencil className="h-4 w-4" /></Button>
                  <Button size="icon" variant="ghost" onClick={() => setDeleting(pc)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                </TableCell>
              </TableRow>
            )) : (
              <TableRow><TableCell colSpan={3} className="py-12 text-center text-muted-foreground">No payment categories yet.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Edit payment category</DialogTitle></DialogHeader>
          {editing && <PaymentCategoryForm defaultValues={editing} submitting={updateMut.isPending} onSubmit={(d) => updateMut.mutate({ id: editing.id, i: d })} />}
        </DialogContent>
      </Dialog>
      <ConfirmDelete
        open={!!deleting}
        ...
        description="This will soft-delete the payment category."
